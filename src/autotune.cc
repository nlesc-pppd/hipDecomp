...
template <typename T>
static std::vector<T> processTimings(hipdecompHandle_t handle, std::vector<T> times, T scale = 1) {
...
void autotuneTransposeBackend(hipdecompHandle_t handle, hipdecompGridDesc_t grid_desc,
  ...
  if (handle->rank == 0) printf("HIPDECOMP: Running transpose autotuning...\n");
  CHECK_MPI(MPI_Barrier(handle->mpi_comm));
  double t_start = MPI_Wtime();

  // Create hip_events for intermediate timings
  ...
  if (autotune_comm) {
    ...
    if (!options->disable_nccl_backends) {
      comm_backend_list.push_back(HIPDECOMP_TRANSPOSE_COMM_NCCL);
  ...
  for (auto& pdim1 : pdim1_list) {
    ...
    hipdecompPencilInfo_t pinfo_x0, pinfo_x3;
    hipdecompPencilInfo_t pinfo_y0, pinfo_y1, pinfo_y2, pinfo_y3;
    hipdecompPencilInfo_t pinfo_z1, pinfo_z2;
    CHECK_HIPDECOMP(hipdecompGetPencilInfo(handle, grid_desc, &pinfo_x0, 0, options->transpose_input_halo_extents[0],
                                           options->transpose_input_padding[0]));
    CHECK_HIPDECOMP(hipdecompGetPencilInfo(handle, grid_desc, &pinfo_x3, 0, options->transpose_output_halo_extents[3],
                                           options->transpose_output_padding[3]));

    CHECK_HIPDECOMP(hipdecompGetPencilInfo(handle, grid_desc, &pinfo_y0, 1, options->transpose_output_halo_extents[0],
                                           options->transpose_output_padding[0]));
    CHECK_HIPDECOMP(hipdecompGetPencilInfo(handle, grid_desc, &pinfo_y1, 1, options->transpose_input_halo_extents[1],
                                           options->transpose_input_padding[1]));
    CHECK_HIPDECOMP(hipdecompGetPencilInfo(handle, grid_desc, &pinfo_y2, 1, options->transpose_output_halo_extents[2],
                                           options->transpose_output_padding[2]));
    CHECK_HIPDECOMP(hipdecompGetPencilInfo(handle, grid_desc, &pinfo_y3, 1, options->transpose_input_halo_extents[3],
                                           options->transpose_input_padding[3]));

    CHECK_HIPDECOMP(hipdecompGetPencilInfo(handle, grid_desc, &pinfo_z1, 2, options->transpose_output_halo_extents[1],
                                           options->transpose_output_padding[1]));
    CHECK_HIPDECOMP(hipdecompGetPencilInfo(handle, grid_desc, &pinfo_z2, 2, options->transpose_input_halo_extents[2],
    ...
    CHECK_HIPDECOMP(hipdecompGetTransposeWorkspaceSize(handle, grid_desc, &num_elements_work));
    int64_t dtype_size;
    CHECK_HIPDECOMP(hipdecompGetDataTypeSize(options->dtype, &dtype_size));
    ...
    if (data_sz_new > data_sz) {
      data_sz = data_sz_new;
      if (data) CHECK_HIP(hipFree(data));
      CHECK_HIP(hipMalloc(&data, data_sz));
      if (need_data2) {
        if (data2) CHECK_HIP(hipFree(data2));
        CHECK_HIP(hipMalloc(&data2, data_sz));
      }
    }
    ...
    if (work_sz_new > work_sz) {
      ...
      if (need_rocshmem) {
        ...
        if (ret == hipErrorOutOfMemory) {
          ...
          grid_desc->config.transpose_comm_backend =
              (need_nccl) ? HIPDECOMP_TRANSPOSE_COMM_NCCL : HIPDECOMP_TRANSPOSE_COMM_MPI_P2P;
          hipdecompResult_t ret = hipdecompMalloc(handle, grid_desc, reinterpret_cast<void**>(&work), work_sz);
          grid_desc->config.transpose_comm_backend = tmp;
        }
#endif
      } else {
        ...
        grid_desc->config.transpose_comm_backend =
            (need_nccl) ? HIPDECOMP_TRANSPOSE_COMM_NCCL : HIPDECOMP_TRANSPOSE_COMM_MPI_P2P;
        if (work) { CHECK_HIPDECOMP(hipdecompFree(handle, grid_desc, work)); }
        CHECK_HIPDECOMP(hipdecompMalloc(handle, grid_desc, reinterpret_cast<void**>(&work), work_sz));
        grid_desc->config.transpose_comm_backend = tmp;
      }
    }
    ...
    for (auto& comm : comm_backend_list) {
      ...
      for (int i = 0; i < options->n_warmup_trials; ++i) {
        if (options->transpose_op_weights[0] != 0.0) {
          CHECK_HIPDECOMP(hipdecompTransposeXToY(handle, grid_desc, data,
        ...
        if (options->transpose_op_weights[1] != 0.0) {
          CHECK_HIPDECOMP(hipdecompTransposeYToZ(handle, grid_desc, data,
        ...
        if (options->transpose_op_weights[2] != 0.0) {
          CHECK_HIPDECOMP(hipdecompTransposeZToY(handle, grid_desc, data,
        ...
        if (options->transpose_op_weights[3] != 0.0) {
          CHECK_HIPDECOMP(hipdecompTransposeYToX(handle, grid_desc, data,
      ...
      for (int i = 0; i < options->n_trials; ++i) {
        CHECK_HIP(hipEventRecord(events[0], 0));
        if (options->transpose_op_weights[0] != 0.0) {
          CHECK_HIPDECOMP(hipdecompTransposeXToY(handle, grid_desc, data,
                                                 options->transpose_use_inplace_buffers[0] ? data : data2, w,
                                                 options->dtype, pinfo_x0.halo_extents, pinfo_y0.halo_extents,
        ...
        CHECK_HIP(hipEventRecord(events[1], 0));
        if (options->transpose_op_weights[1] != 0.0) {
          CHECK_HIPDECOMP(hipdecompTransposeYToZ(handle, grid_desc, data,
                                                 options->transpose_use_inplace_buffers[1] ? data : data2, w,
                                                 options->dtype, pinfo_y1.halo_extents, pinfo_z1.halo_extents,
        ...
        CHECK_HIP(hipEventRecord(events[2], 0));
        if (options->transpose_op_weights[2] != 0.0) {
          CHECK_HIPDECOMP(hipdecompTransposeZToY(handle, grid_desc, data,
                                                 options->transpose_use_inplace_buffers[2] ? data : data2, w,
                                                 options->dtype, pinfo_z2.halo_extents, pinfo_y2.halo_extents,
        ...
        CHECK_HIP(hipEventRecord(events[3], 0));
        if (options->transpose_op_weights[3] != 0.0) {
          CHECK_HIPDECOMP(hipdecompTransposeYToX(handle, grid_desc, data,
                                                 options->transpose_use_inplace_buffers[3] ? data : data2, w,
                                                 options->dtype, pinfo_y3.halo_extents, pinfo_x3.halo_extents,
        ...
        CHECK_HIP(hipEventRecord(events[4], 0));
        CHECK_HIP(hipDeviceSynchronize());
        CHECK_MPI(MPI_Barrier(handle->mpi_comm));

        if (options->transpose_op_weights[0] != 0.0)
          CHECK_HIP(hipEventElapsedTime(&trial_xy_times[i], events[0], events[1]));
        if (options->transpose_op_weights[1] != 0.0)
          CHECK_HIP(hipEventElapsedTime(&trial_yz_times[i], events[1], events[2]));
        if (options->transpose_op_weights[2] != 0.0)
          CHECK_HIP(hipEventElapsedTime(&trial_zy_times[i], events[2], events[3]));
        if (options->transpose_op_weights[3] != 0.0)
          CHECK_HIP(hipEventElapsedTime(&trial_yx_times[i], events[3], events[4]));
      ...
      // Clear HIP graph cache between backend/process decomposition pairs
      ...
      if (handle->rank == 0) {
        if (skip_case) {
          printf("HIPDECOMP:\tgrid: %d x %d, backend: %s \n"
                 "HIPDECOMP:\t(skipped) \n",
          ...
          printf("HIPDECOMP:\tgrid: %d x %d, backend: %s \n"
                 "HIPDECOMP:\tTotal time min/max/avg/std [ms]: %f/%f/%f/%f\n"
                 "HIPDECOMP:\t           min/max/avg/std [ms]: %f/%f/%f/%f (weighted)\n"
                 "HIPDECOMP:\tTransposeXY time min/max/avg/std [ms]: %f/%f/%f/%f%s\n"
                 "HIPDECOMP:\tTransposeYZ time min/max/avg/std [ms]: %f/%f/%f/%f%s\n"
                 "HIPDECOMP:\tTransposeZY time min/max/avg/std [ms]: %f/%f/%f/%f%s\n"
                 "HIPDECOMP:\tTransposeYX time min/max/avg/std [ms]: %f/%f/%f/%f%s\n",
                 grid_desc->config.pdims[0], grid_desc->config.pdims[1],
                 hipdecompTransposeCommBackendToString(grid_desc->config.transpose_comm_backend), times[0], times[1],
  ...
  if (need_data2) { CHECK_HIP(hipFree(data2)); }
  ...
  CHECK_MPI(MPI_Bcast(&comm_backend_best, sizeof(hipdecompTransposeCommBackend_t), MPI_CHAR, 0, handle->mpi_comm));
  ...
  if (handle->rank == 0) {
    printf("HIPDECOMP: SELECTED: grid: %d x %d, backend: %s, Avg. time (weighted) [ms]: %f\n",
           grid_desc->config.pdims[0], grid_desc->config.pdims[1],
           hipdecompTransposeCommBackendToString(grid_desc->config.transpose_comm_backend), t_best);
  }

  CHECK_MPI(MPI_Barrier(handle->mpi_comm));
  double t_end = MPI_Wtime();
  if (handle->rank == 0) printf("HIPDECOMP: transpose autotuning time [s]: %f\n", t_end - t_start);
  ...
  CHECK_HIP(hipMalloc(&tmp1, per_rank_size * handle->nranks * sizeof(*tmp1)));
  CHECK_HIP(hipMalloc(&tmp2, per_rank_size * handle->nranks * sizeof(*tmp2)));
  CHECK_MPI(MPI_Alltoall(tmp1, per_rank_size, MPI_CHAR, tmp2, per_rank_size, MPI_CHAR, handle->mpi_comm));
  CHECK_MPI(MPI_Barrier(handle->mpi_comm));
  CHECK_HIP(hipFree(tmp1));
  CHECK_HIP(hipFree(tmp2));
...
void autotuneHaloBackend(hipdecompHandle_t handle, hipdecompGridDesc_t grid_desc,
                         const hipdecompGridDescAutotuneOptions_t* options) {
  if (handle->rank == 0) {
    printf("HIPDECOMP: Running halo autotuning...\n");
    printf("HIPDECOMP: Autotune halo axis: %s\n",
  ...
  for (auto& pdim1 : pdim1_list) {
    ...
    CHECK_HIPDECOMP(hipdecompGetPencilInfo(handle, grid_desc, &pinfo, options->halo_axis, options->halo_extents,
    ...
    // Allocate test data
    int64_t num_elements_work;
    CHECK_HIPDECOMP(hipdecompGetHaloWorkspaceSize(handle, grid_desc, options->halo_axis, options->halo_extents,
                                                  &num_elements_work));
    int64_t dtype_size;
    CHECK_HIPDECOMP(hipdecompGetDataTypeSize(options->dtype, &dtype_size));
    ...
    for (auto& comm : comm_backend_list) {
      ...
      for (int i = 0; i < options->n_warmup_trials; ++i) {
        for (int dim = 0; dim < 3; ++dim) {
          ...
          case 0:
            CHECK_HIPDECOMP(hipdecompUpdateHalosX(handle, grid_desc, d, w, options->dtype, pinfo.halo_extents,
                                                  options->halo_periods, dim, pinfo.padding, 0));
            break;
          case 1:
            CHECK_HIPDECOMP(hipdecompUpdateHalosY(handle, grid_desc, d, w, options->dtype, pinfo.halo_extents,
                                                  options->halo_periods, dim, pinfo.padding, 0));
            break;
          case 2:
            CHECK_HIPDECOMP(hipdecompUpdateHalosZ(handle, grid_desc, d, w, options->dtype, pinfo.halo_extents,
                                                  options->halo_periods, dim, pinfo.padding, 0));
            break;
          }
        }
      ...
      if (handle->rank == 0) {
        if (skip_case) {
          printf("HIPDECOMP:\tgrid: %d x %d, halo backend: %s \n"
                 "HIPDECOMP:\t(skipped) \n",
                 grid_desc->config.pdims[0], grid_desc->config.pdims[1],
          ...
          printf("HIPDECOMP:\tgrid: %d x %d, halo backend: %s \n"
                 "HIPDECOMP:\tTotal time min/max/avg/std [ms]: %f/%f/%f/%f\n",
                 grid_desc->config.pdims[0], grid_desc->config.pdims[1],
                 hipdecompHaloCommBackendToString(grid_desc->config.halo_comm_backend), times[0], times[1], times[2],
                 times[3]);
        }
      }
  ...
  // Set handle to best option (broadcast from rank 0 for consistency)
  CHECK_MPI(MPI_Bcast(&comm_backend_best, sizeof(hipdecompHaloCommBackend_t), MPI_CHAR, 0, handle->mpi_comm));
  CHECK_MPI(MPI_Bcast(pdims_best, 2 * sizeof(int), MPI_INT, 0, handle->mpi_comm));
  ...
  if (handle->rank == 0) {
    printf("HIPDECOMP: SELECTED: grid: %d x %d, halo backend: %s, Avg. time [ms]: %f\n", grid_desc->config.pdims[0],
           grid_desc->config.pdims[1], hipdecompHaloCommBackendToString(grid_desc->config.halo_comm_backend), t_best);
  }

  CHECK_MPI(MPI_Barrier(handle->mpi_comm));
  double t_end = MPI_Wtime();
  if (handle->rank == 0) printf("HIPDECOMP: halo autotuning time [s]: %f\n", t_end - t_start);
